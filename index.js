...
app.post('/runquery', function(req, res) {
  ...
  switch(topic) {
    ...
    case 'q3':
      ...
      break;
    case 'q4':
      query = "SELECT agent_id,fname,lname,NOP_sale,total_saleAmount from agent ORDER BY total_saleAmount desc LIMIT 1";
      break;
    case 'q5':
      query = "SELECT agent.agent_id, agent.fname, agent.lname, AVG(properties.price) AS avg_price, AVG(YEAR(NOW()) - properties.year_of_listing) AS avg_years_on_market FROM properties JOIN agent ON properties.agent_id = agent.agent_id WHERE  properties.year_of_sale=2023 AND properties.status='sold' GROUP BY agent.agent_id;";
      break;
    case 'q6':
